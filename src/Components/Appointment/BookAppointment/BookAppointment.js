...
const BookAppointment = ({date}) => {
    ...
    return (
        <section>
            <h1 className='text-center text-info mb-4'>Available Appointment On {date.toDateString()}</h1>

            <div className="row ">
                {
                    bookingData.map(data =><BookingCard date={date} key={data.id} data={data}></BookingCard>)
                }
            </div>
        </section>
    );
